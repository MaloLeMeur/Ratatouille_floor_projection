#!/usr/bin/env python3
"""
Video Player controlled by PIR detector via WiFi
Replaces spacebar with motion detection
"""
...
# Config setup
SETTINGS = {
    'res': (1280, 720),       # Adjusted for Raspberry Pi 2
    'no_input_secs': 7,       # wait for no motion detection
    'motion_debounce_ms': 200, # debounce for motion detection
    'fps': 30,                # smooth input
    'fade_ms': 500,          # 0.5 sec fades
    'vid_path': 'projection version04.mp4',   # single video
...
class VidPlayer:
    ...
    def fade_to_black(self):
        ...
        self.player.video_set_adjust_float(vlc.VideoAdjustOption.Contrast, 0.0)
    ...
    def fade_from_black(self):
        ...
        self.is_fading = True
        steps = int(SETTINGS['fade_ms'] / (2000 / SETTINGS['fps']))
        for step in range(steps):
